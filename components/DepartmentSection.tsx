...
export const DepartmentSection: React.FC<DepartmentSectionProps> = ({ votes }) => {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8 mb-8">
      {/* Bar Chart */}
      <div className="bg-slate-800 border border-slate-600 rounded-2xl p-5 md:p-8 shadow-lg">
        <h2 className="text-xl md:text-2xl font-bold text-teal-400 mb-6 flex items-center gap-3 border-b border-slate-700 pb-4">
          🏢 Votes by Department
        </h2>
        <div className="h-[250px] md:h-[300px]">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={data} margin={{ top: 20, right: 10, left: 0, bottom: 0 }}>
              ...
              />
              <Bar dataKey="count" fill="#14b8a6" radius={[6, 6, 0, 0]} barSize={30}>
                <LabelList dataKey="count" position="top" fill="#f1f5f9" fontWeight="bold" fontSize={12} />
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        </div>
      ...
      {/* Details List */}
      <div className="bg-slate-800 border border-slate-600 rounded-2xl p-5 md:p-8 shadow-lg flex flex-col justify-between">
        <h2 className="text-xl md:text-2xl font-bold text-teal-400 mb-6 flex items-center gap-3 border-b border-slate-700 pb-4">
          📋 Department Details
        </h2>
