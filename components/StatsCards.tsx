...
export const StatsCards: React.FC<StatsCardsProps> = ({ totalVotes, validVotes, abstentions, candidateCount }) => {
  const cards = [
    { label: "Total Votes", value: totalVotes, sub: "Cast", delay: "delay-0" },
    { label: "Valid Votes", value: validVotes, sub: "Active", delay: "delay-100" },
    { label: "Abstentions", value: abstentions, color: "text-slate-400", sub: "Recused", delay: "delay-200" },
    { label: "Candidates", value: candidateCount, sub: "Running", delay: "delay-300" },
  ];
  ...
  return (
    ...
    <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-2 md:gap-4 mb-4 md:mb-8">
      {cards.map((card, index) => (
        <div 
          key={index} 
          className={`glass-panel rounded-xl p-3 md:p-6 shadow-lg shadow-teal-900/20 bg-slate-800/80 transition-all duration-300 group relative overflow-hidden animate-fade-in-up ${card.delay}`}
        >
          ...
          <div className="relative z-10 flex flex-col items-center md:items-start text-center md:text-left">
            ...
            <p className={`text-2xl md:text-5xl font-black tracking-tighter drop-shadow-sm transition-all duration-300 origin-left ${card.color || 'text-white'}`}>
              {card.value}
            </p>
