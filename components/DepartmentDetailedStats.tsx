            ...
            <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-2 md:gap-6">
                {deptData.map(({ dept, total, breakdown }, idx) => (
                    <ScrollReveal key={dept} delay={idx * 100} width="100%">
                        <div className="bg-slate-800/60 border border-teal-500/30 rounded-xl p-3 md:p-6 shadow-lg relative overflow-hidden flex flex-col transition-all duration-300 h-full">
                        {/* Header */}
                        <div className="flex justify-between items-end mb-3 md:mb-5 pb-2 md:pb-4 border-b border-white/5">
                                ...
                                <span className="text-teal-400 font-mono text-[10px] md:text-sm font-bold bg-teal-500/10 px-1.5 md:px-3 py-0.5 md:py-1 rounded border border-teal-500/20">
                                    <CountUp end={total} />
                                </span>
                        </div>
                        ...
                        <div className="w-full bg-slate-800/50 rounded-full h-3 md:h-5 mb-3 md:mb-6 flex overflow-hidden border border-white/5">
                                {breakdown.map((item, i) => (
                                    <AnimatedBar 
                                        ...
                                        style={{ 
                                            flexShrink: 0
                                            // Fixed: Removed flexBasis/maxWidth so AnimatedBar can control the width
                                        }}
                                        children={null}
                                    />
                                ))}
                                ...
                                {breakdown.map((item) => (
                                    ...
                                    <div key={item.candidate} className="flex justify-between items-center text-[10px] md:text-sm bg-white/5 p-1 md:p-2 rounded-lg transition-colors -mx-1 md:-mx-2">
                                        ...
                                        <div className="flex items-center gap-1 md:gap-3 flex-shrink-0">
                                            <span className="font-bold text-white text-xs md:text-lg tabular-nums">
