...
export const CountdownTimer: React.FC<CountdownTimerProps> = ({ onStatusChange }) => {
  const [timeLeft, setTimeLeft] = useState<{
    ...
    mode: 'ANNOUNCEMENT';
    hasStarted: boolean;
  }>({ days: 0, hours: 0, minutes: 0, seconds: 0, isClosed: true, mode: 'ANNOUNCEMENT', hasStarted: false });

  const [dateTimeInfo, setDateTimeInfo] = useState<{date: string, time: string}>({ date: "", time: "" });

  // Fixed Results Announcement Date: Dec 31, 2025 15:00 (Brasilia Time)
  const ANNOUNCEMENT_TARGET = "2025-12-31T15:00:00-03:00";

  useEffect(() => {
    // Since we are now exclusively in the announcement/results phase,
    // the election is always considered "closed" for the rest of the app logic.
    if (onStatusChange) {
      onStatusChange(true);
    }
  }, [onStatusChange]);
  ...
  return (
    <ScrollReveal delay={50}>
      <div 
        className="glass-panel rounded-xl p-3 md:px-6 md:py-4 mb-4 md:mb-6 relative overflow-hidden transition-colors duration-500 bg-blue-950/40 border-blue-500/30 shadow-[0_0_30px_rgba(30,58,138,0.2)]"
      >
